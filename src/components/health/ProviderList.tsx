...
const ProviderList: React.FC<ProviderListProps> = ({ entries, setEntries }) => {
  ...
  const handleImport = async (importedData: HealthProvider[]) => {
  ...
  const handlePrint = () => {
    // Print all providers (or filtered entries if search is active)
    const entriesToPrint = searchQuery ? filteredEntries : safeEntries;
    printService.printHealthProviders(entriesToPrint, 'Health Providers');
  };
  ...
  return (
    <div>
      ...
      <ImportExport
        ...
        exportFunction={csvService.exportHealthProvidersToCsv}
        importFunction={csvService.importHealthProvidersFromCsv}
        filename={`health-providers-${new Date().toISOString().split('T')[0]}.csv`}
      />
